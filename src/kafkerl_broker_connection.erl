...
%% API
-export([add_buffer/2, clear_buffers/1, delete_buffer/2, fetch/4, stop_fetch/3]).
...
delete_buffer(ServerRef, Buffer) ->
    gen_server:call(ServerRef, {delete_buffer, Buffer}).
...
handle_call({delete_buffer, Buffer}, _From, State = #state{buffers = Buffers}) ->
    {reply, ok, State#state{buffers = Buffers -- [Buffer]}};
...
handle_errors(Errors, Messages, Name) ->
    lager:warning("error ~p in handling errors", [Errors]),
    F = fun(E) -> handle_error(E, Messages, Name) end,
    case lists:filtermap(F, Errors) of
        [] -> ignore;
        L ->
            lager:warning("found errors when sending ~p, requesting metadata", [L]),
            {request_metadata, L}
    end.
